...
class MeanShift(nn.Conv2d):
    def __init__(self, rgb_range, rgb_mean, rgb_std, sign=-1):
        ...
        self.weight.data = torch.eye(3).reshape([3, 3, 1, 1])
        self.weight.data = self.weight / (std.reshape([3, 1, 1, 1]))
        ...
        dual_block = [
            nn.Sequential(
                nn.Conv2d(in_channels,
                          ...
                          stride=(2,),
                          padding=1,
                          bias=False), nn.LeakyReLU(negative_slope=negval))
        ]
...
class DRNGenerator(nn.Module):
    ...
    def forward(self, x):
        ...
        for idx in range(self.phase):
            ...
            x = torch.concat((x, copies[self.phase - idx - 1]), 1)
            # output sr imgs
...
if __name__ == '__main__':
    gen = DRNGenerator((2, 4))
    t = torch.rand((2, 3, 128, 128))
    gen(t)