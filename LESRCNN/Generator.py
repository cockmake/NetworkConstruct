...
class MeanShift(nn.Module):
    def __init__(self, mean_rgb, sub):
        ...
        self.shifter.weight.data = torch.eye(3).reshape([3, 3, 1, 1])
    ...
    def __init__(self, n_channels, scale, group=1):
        ...
        if scale == 2 or scale == 4 or scale == 8:
            for _ in range(int(math.log(scale, 2))):
                modules += [
                    nn.Conv2d(n_channels, 4 * n_channels, (3, 3), (1, 1), 1, groups=group)
                ]
                modules += [nn.PixelShuffle(2)]
        elif scale == 3:
            modules += [
                nn.Conv2d(n_channels, 9 * n_channels, (3, 3), (1, 1), 1, groups=group)
            ]
...
class LESRCNNGenerator(nn.Module):
    """Construct a Resnet-based generator that consists of residual blocks
    between a few down-sampling / upsampling operations.
    Args:
        scale (int): scale of upsample.
        multi_scale (bool): Whether to train multiscale model.
